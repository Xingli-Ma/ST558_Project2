...
Here we displayed the training data set and gave a numerical summary for all variables.    
...
First, we compared number of Users by whether day is workingday or not.
...
Second, we compared number of Users by weather conditions (1: Clear; 2: Mist + Cloudy; 3: Light Snow + Light Rain; 4: Heavy Rain + Ice Pallets + Thunderstorm + Mist, Snow + Fog). 
...
The three histograms show the distribution of the number of bike users in different season.
...
Boosting trees grown sequentially. Each subsequent tree is grown on a modified version of original data. Predictions updated as trees grown. They are slowly training trees, so that not overfit.
...
Using each of the four model make predictions on the testing data set.
...
Select the best model that have the smallest RMSE value.
